---
title: "Data Analyst - Desafio"
subtitle: "Análise Exploratória de Dados"
institute: "Como melhorar a experiência de aprendizagem dos estudantes?"
author: "Luciano O. Batista"
date: "<br><br><br><font size='5'>.right[.small[29/08/2020]]</font>"
output:
...
```{r libs}
# Caso queira recompilar o slide em seu ambiente
# é precisa ter instalado no sistema algumas libs:
# install.packages("xaringan")
# install.packages("xaringanExtra")
# install.packages("xaringanthemer")

# Carregando as Libs utilizadas na EDA
library(xaringan)
...
# Ofertas dos cursos: Provas, Período de oferta e recurso online
assessments <- read_csv("data/university/assessments.csv")
courses <- read_csv("data/university/courses.csv")
vle <- read_csv("data/university/vle.csv")

# Fatores Demográficos
student_info <- read_csv("data/university/studentInfo.csv")

# Dados sobre interação dos estudantes
student_registration <- read_csv("data/university/studentRegistration.csv")
student_assessment <- read_csv("data/university/studentAssessment.csv")
# Essa tabela é bem grande e pode consumir muita memória
student_vle <- data.table::fread("data/university/studentVle.csv")
```
...
```{r xaringan-tile-view}
xaringanExtra::use_tile_view()
```

```{r xaringan-animate-css}
...
.left[Observou-se que sempre que um curso é ofertado  no **<span style="color:#12A661">primeiro período</span>** do ano ele possui menor carga horária total (em dias), comparado ao **<span style="color:#1495B3">segundo período</span>**.]
...
assem_demog_info_v1_fct_tbl %>% 
  ggplot(aes(x = score,
             y = fct_rev(code_presentation),
             fill = stat(x))) + 
  geom_density_ridges_gradient(gradient_lwd = 1., size = .25) +
  facet_wrap(~code_module) +
  scale_x_continuous(expand = c(0, 0)) +
...
htmltools::save_html(interactive_plot_gender, file = "interactive_plot_gender.html")
htmltools::save_html(interactive_plot_ageband, file = "interactive_plot_ageband.html")
